...
static int syna_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		if (ts->snr_read_support) {
			if (snr->doing && points[idx].x && points[idx].y) {
				snr->x = points[idx].x;
				snr->y = points[idx].y;
				snr->channel_x = snr->x * tcm_hcd->hw_res->TX_NUM / ts->resolution_info.max_x;
				snr->channel_y = snr->y * tcm_hcd->hw_res->RX_NUM / ts->resolution_info.max_y;
				TPD_INFO("%s: snr: [%d %d, %d] {%d %d}\n", __func__,
					snr->x, snr->y, idx, snr->channel_x, snr->channel_y);
			}
		}
	}
...
static int syna_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_NORMAL:
		...
		/* reinit confit_complete to avoid echo 1/2 tp_fw_update failed sometimes*/
		reinit_completion(&tcm_hcd->config_complete);
...
static void syna_tcm_black_screen_test(void *chip_data, char *message)
{
	...
	sprintf(data_buf, "/sdcard/TpTestReport/tp_testlimit_gesture_%02d%02d%02d-%02d%02d%02d-utc.csv",
...
static void syna_tcm_format_print_snr(struct seq_file *s, struct syna_tcm_hcd *tcm_hcd, int index)
{
	int temp;
	unsigned int offset;
	unsigned int rx_num;
	...
	tx_num = tcm_hcd->hw_res->TX_NUM;
	rx_num = tcm_hcd->hw_res->RX_NUM;
	TPD_INFO("tx_num = %d tx_num = %d\n", tx_num, rx_num);

	pdata_16 = (short *)&test_hcd->report.buf[0];
	offset = (rx_num - snr->channel_y - 1) * tx_num + snr->channel_x;
	temp = *(pdata_16 + offset);
	TPD_INFO("pdata_16 = %d offset = %d temp = %d\n", *pdata_16, offset, temp);
	...
	TPD_INFO("snr-cover [%d %d] %d.. %d += %d pdata_16:%d\n",
		snr->channel_x, snr->channel_y, index, snr->sum, temp, *pdata_16);
...
static void syna_delta_snr_read(struct seq_file *s, void *chip_data, uint32_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		syna_tcm_format_print_snr(s, tcm_hcd, i);
