...
static bool sia81xx_is_chip_en(sia81xx_dev_t *sia81xx)
{
	if (0 == sia81xx->disable_pin) {
		if (sia81xx->chip_type == CHIP_TYPE_SIA815T ||
					sia81xx->chip_type == CHIP_TYPE_SIA8159 ) {
			if ((SIA81XX_ENABLE_LEVEL == gpio_get_value(sia81xx->rst_pin))
				&& sia81xx_regmap_get_chip_en(sia81xx->regmap, sia81xx->chip_type))
				return true;
		} else if (SIA81XX_ENABLE_LEVEL == gpio_get_value(sia81xx->rst_pin))
			return true;
	} else {
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if (is_chip_type_supported(sia81xx->chip_type) &&
		...
		if (0 == sia81xx->disable_pin) {
			...
			if(CHIP_TYPE_SIA8100 == sia81xx->chip_type){
				/* power up chip */
...
static ssize_t sia81xx_chip_show(
	...
	char* buf)
{
	sia81xx_dev_t *sia81xx = (sia81xx_dev_t *)dev_get_drvdata(dev);
	char value[8];
...
static DEVICE_ATTR(sia81xx_chip, S_IRUGO, sia81xx_chip_show, NULL);
#endif
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sia81xx_is_chip_en(sia81xx)){
		if(CHIP_TYPE_SIA8109 == sia81xx->chip_type){
			...
			pr_debug("[debug addcheck]sia81xx_reboot \r\n");
		}
		else{
			sia81xx_resume(sia81xx);
			pr_debug("[debug addcheck]sia81xx_resume \r\n");
		}
	}
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(sia81xx, &sia81xx_list, list) {
		if (NULL != sia81xx) {
			ucontrol->value.integer.value[0] = sia81xx->scene;
			pr_debug("[debug][%s] %s: ucontrol = %ld channel = %d \r\n",
				LOG_FLAG, __func__, ucontrol->value.integer.value[0],sia81xx->channel_num);
		}
	}
	mutex_unlock(&sia81xx_list_mutex);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(sia81xx, &sia81xx_list, list) {
		if(NULL != sia81xx) {
			pr_debug("[debug][%s] %s: ucontrol = %d, rst = %d, channle = %d \r\n", 
			...
			if(AUDIO_SCENE_NUM <= scene) {
				...
				sia81xx->scene = scene;
				if(sia81xx->scene == AUDIO_SCENE_VOICE){
					sia81xx->scene = AUDIO_SCENE_PLAYBACK;
				}
			}
			if (sia81xx_is_chip_en(sia81xx)){
	...
	list_for_each_entry(sia81xx, &sia81xx_list, list) {
		if(NULL != sia81xx) {
			if(CHIP_TYPE_SIA8152 == sia81xx->chip_type) {
				if (0 != sia81xx_regmap_read(sia81xx->regmap, sia8152x_addr, 1, &val)) {
				...
				if (0 != sia81xx_regmap_read(sia81xx->regmap, sia81xx_addr, 1, &val)) {
		...
		if(NULL != sia81xx) {
			...
			if(CHIP_TYPE_SIA8152 == sia81xx->chip_type) {
				if(sia81xx->channel_num == 0) {
					if (0 != sia81xx_regmap_write(sia81xx->regmap, sia8152x_addr, 1, voltage_sia8152x_L+index)) {
						pr_err("[err][%s] %s: regmap_write \r\n",LOG_FLAG, __func__);
					}
				} else {
					if (0 != sia81xx_regmap_write(sia81xx->regmap, sia8152x_addr, 1, voltage_sia8152x_R+index)) {
						pr_err("[err][%s] %s: regmap_write \r\n",LOG_FLAG, __func__);
					}
				}
			} else {
				if (0 != sia81xx_regmap_write(sia81xx->regmap, sia81xx_addr, 1, voltage_sia81xx+index)) {
					pr_err("[err][%s] %s: regmap_write \r\n",LOG_FLAG, __func__);
				}
			}
		}
...
static int check_sia81xx_status(sia81xx_dev_t *sia81xx)
{
	...
	sia81xx_set_rst(sia81xx, SIA81XX_ENABLE_LEVEL);

	ret = sia81xx_regmap_check_chip_id(
			sia81xx->regmap,  sia81xx->chip_type);

	sia81xx_set_rst(sia81xx, SIA81XX_DISABLE_LEVEL);

	return ret;
}
#endif

static void sia81xx_set_rst(sia81xx_dev_t *sia81xx, int value)
{
	if (0 == sia81xx->disable_pin && !IS_SUPPORT_OWI_TYPE(sia81xx->chip_type)) {
		gpio_set_value(sia81xx->rst_pin, value);
	...
	const struct i2c_device_id *id)
{
	...
	if ((detect_i2c_slave(regmap,chip_type) == 0) && (speaker_device == NULL)) {
